...
class TwitterHandleLanding extends Component {
  constructor (props) {
    super(props);
    this.state = {
      activeRoute: '',
      hasProcessedDeferredImages: false,
      twitterHandle: '',
    };
  }
  ...
  onTwitterStoreChange () {
    ...
    const { hasProcessedDeferredImages } = this.state;
    ...
    const {
      ...
      twitter_profile_banner_url_https: twitterProfileBannerUrlHttps,
      we_vote_hosted_profile_image_url_large: weVoteHostedProfileImageUrlLarge,
      status,
    } = TwitterStore.get();
    // console.log('TwitterStore.get():', TwitterStore.get());

    if (typeof twitterFollowersCount !== 'number') {
      twitterFollowersCount = 0;
    }

    const processDeferredImages = !hasProcessedDeferredImages;
    // const processDeferredImages = false;
    if (processDeferredImages && twitterId && !weVoteHostedProfileImageUrlLarge) {
      const twitterImageLoadInfo = {
        organization_we_vote_id: ownerWeVoteId,
        success: true,
        twitter_id: twitterId,
        twitter_name: twitterName,
        twitter_profile_banner_url_https: twitterProfileBannerUrlHttps,
        twitter_profile_image_url_https: twitterPhotoUrl,
        twitter_screen_name: twitterHandle,
        // voter_we_vote_id_for_cache: twitterImageLoadInfo.voter_we_vote_id_for_cache,
      };
      // console.log('twitterImageLoadInfo:', twitterImageLoadInfo);
      TwitterActions.twitterProcessDeferredImages(twitterImageLoadInfo);
      this.setState({
        hasProcessedDeferredImages: true,
      });
    }
    this.setState({
      kindOfOwner,
  ...
  render () {
    ...
    if (kindOfOwner === 'CANDIDATE') {
      ...
      return <PageContentContainer>Candidate</PageContentContainer>;
      ...
      return (
        <OrganizationVoterGuide
          ...
          organizationWeVoteId={ownerWeVoteId}
          // {...this.props}
          params={params}
      ...
      return <PageContentContainer>UnknownTwitterAccount</PageContentContainer>;
    } else {
          ...
          <Suspense fallback={<></>}>
            <DelayedLoad showLoadingText waitBeforeShow={2000}>
              ...
              >
                ...
                <h3 className="h3">Endorser not found</h3>
...
export default withStyles(styles)(TwitterHandleLanding);
